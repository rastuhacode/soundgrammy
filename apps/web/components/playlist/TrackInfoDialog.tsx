...
export function TrackInfoDialog({
  ...
  const metadataQuery = useQuery({
    ...trpc.tracks.metadata.queryOptions({ trackId: track?.id ?? 0 }),
    enabled: open && track !== null,
  });
  const metadata = metadataQuery.data;
  const loading = metadataQuery.isPending;
  const error = metadataQuery.error?.message ?? null;
  ...
  const performer = metadata?.track.performer ?? track?.performer ?? "Unknown Artist";

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[min(85vh,720px)] flex-col overflow-hidden sm:max-w-lg">
                ...
                <>
                  <section className="space-y-2">
                    ...
                    <dl className="space-y-2 rounded-lg border border-border bg-card/50 p-3">
                      <MetadataRow label="Title" value={metadata.track.title ?? "—"} />
                      <MetadataRow label="Artist" value={metadata.track.performer ?? "—"} />
                      <MetadataRow
                        label="Duration"
                        value={formatDuration(metadata.track.duration)}
                      />
                      <MetadataRow label="MIME type" value={metadata.track.mimeType ?? "—"} />
                      <MetadataRow
                        label="File size"
                      ...
                      />
                      <MetadataRow label="Added" value={metadata.track.createdAt} />
                    </dl>
                  </section>
                  ...
                  <section className="space-y-2">
                    ...
                    <dl className="space-y-2 rounded-lg border border-border bg-card/50 p-3">
                      <MetadataRow label="Document ID" value={metadata.document.id} />
                      <MetadataRow label="DC ID" value={metadata.document.dcId} />
                      <MetadataRow label="MIME type" value={metadata.document.mimeType ?? "—"} />
                      <MetadataRow
                        label="Size"
                      ...
                      <MetadataRow
                        ...
                        value={
                          metadata.document.hasRemoteThumb
                            ...
                            : metadata.document.hasInlineThumb
                              ? "Inline thumb"
                              : "Embedded / none"
                        }
                      />
                  ...
                  <section className="space-y-2">
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        from
                        {" "}
                        {metadata.document.attributesSource}
                      </span>
                    </div>
