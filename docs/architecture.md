...
- **Uncached (streamed)**: `get_track_source` returns `stream`; the private HTML engine attaches `<audio>` to a MediaSource object URL and appends bytes via `read_stream_range` as the download ledger grows (`src/hooks/audio/html/mse-session.ts`). Full track duration is set on the `MediaSource` from metadata. Buffer UI receives playable time ranges from the engine; downloaded byte ranges are not painted as playable MSE coverage.
